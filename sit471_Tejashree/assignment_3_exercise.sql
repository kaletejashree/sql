...
insert into actor(act_fname,act_lname,act_gender) values ('John', 'Doe', 'M'),('Tej', 'Kale', 'F'),('ritz', 'patel', 'M'),('Janvhi', 'Dongriya', 'F');
insert into genre(gen_title) values ( 'Action'),('Comedy'),('Drama'),('History');
...
insert into movie_direction values (904, 902),(903, 902),(902, 903),(901, 904);
...
select distinct d.dir_id,d.dir_fname,d.dir_lname,r.rev_stars
from director as d join movie_direction as m
on d.dir_id=m.dir_id
join rating as r
on m.mov_id=r.mov_id
...
create function worstd()
returns table
as
return(
select top 1 d.dir_id,d.dir_fname,r.rev_stars, avg(r.num_o_rating) as rat from director as d
inner join movie_direction m
on d.dir_id=m.dir_id
inner join rating as r
on r.mov_id=m.mov_id
group by d.dir_id,d.dir_fname,r.rev_stars
order by rat);
select * from worstd();

--17.  create a function to get worst actor according to movie rating
create function worstactor()
returns table
as
return(
SELECT TOP 1
    ...
    ORDER BY
        AverageRating);
select * from worstactor();

--18. create a parameterized stored procedure which accept genre and give movie accordingly 
create procedure parasp( @generetitle varchar(20))
as
begin
select M.mov_id, M.mov_title, M.mov_year, M.mov_time, M.mov_lang, M.mov_dt_rel, M.mov_release_country
 from  movie M join movie_genres MG ON M.mov_id = MG.mov_id
 join genre G ON MG.gen_id = G.gen_id
where G.gen_title = @generetitle;
end;

exec parasp @generetitle='Action';
...
create function getratemovie()
returns table
as
return(
select top 3 rw.rev_id,rw.rev_name,r.num_o_rating from reviewer as rw inner join rating as r
on rw.rev_id=r.rev_id
inner join movie as m on m.mov_id=r.mov_id
group by rw.rev_id,rw.rev_name,r.num_o_rating
order by count(r.num_o_rating) desc);

select * from getratemovie();
...
create function getbestmovie()
returns table
as
return(
select top 1 g.gen_id,g.gen_title,r.num_o_rating from genre as g inner join movie_genres as mg
on g.gen_id=mg.gen_id
inner join movie as m on m.mov_id=mg.mov_id
inner join rating as r on r.mov_id=mg.mov_id
order by r.num_o_rating desc);
select * from getbestmovie();

--26. create a function which accepts genre and suggests best director according to ratings. 
create function bestdirector()
returns table
as
return(
select top 1 g.gen_id,g.gen_title,d.dir_id, r.num_o_rating from genre as g inner join movie_genres as mg
on g.gen_id=mg.gen_id
inner join movie as m on m.mov_id=mg.mov_id
...
--27. create a function that accepts a genre and give random movie according to genre
 select top 1 M.mov_id,M.mov_title,M.mov_year, M.mov_time, M.mov_lang, M.mov_dt_rel, M.mov_release_country
 from movie M join movie_genres MG ON M.mov_id = MG.mov_id
join genre G ON MG.gen_id = G.gen_id
...
/*-----------------------------------------------------------------------------------------------
create a stored proc to get list of movies which is 3 years old and having rating greater than 5*/

create procedure getmovie1
as
begin
 declare @currentyear int;
 set @currentyear=YEAR(getDate());
 select m.mov_id,m.mov_title,m.mov_year,m.mov_time,m.mov_lang,m.mov_dt_rel,m.mov_release_country
 from movie as m inner join rating as r on m.mov_id=r.mov_id
 where @currentyear -m.mov_year<=3 and r.num_o_rating>5;
end;
exec getmovie1;


drop procedure directorlist;



-- create a stored proc to get list of all directors who have directed more then 2 movies
create procedure directorlist
as
begin
   select d.dir_id,d.dir_fname,d.dir_lname ,count(m.mov_id)from director as d inner join 
...
as
begin 
select * from director as d inner join movie_direction as md
on d.dir_id=md.dir_id
inner join rating as r on r.mov_id=md.mov_id
where r.num_o_rating>3;
end;
exec alldirectors;
...
--19. get list of movies that start with 'a' and end with letter 'e' and movie released before 2015
select * from movie where mov_title like 'a%e' and year(mov_dt_rel)<2015;
